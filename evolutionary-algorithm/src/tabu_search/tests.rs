...
pub fn get_tabu_search_best_three(instance: &str) -> Result<Vec<Box<dyn Solver>>> {
    let iteration_info_headers = vec![
        "configuration".to_string(),
        "iteration".to_string(),
        "tabu_size".to_string(),
    ...
    Ok(vec![
        Box::new(
            TabuSearchBuilder::default()
                .iterations(1000)
                .tabu_list_size(40)
                .neighborhood_operator(Box::new(SwapNeighborhoodOperator::new(30)))
                .logger(Box::new(CSVLogger::new(
                    instance,
                    Some(iteration_info_headers),
                )))
        ...
        Box::new(
            TabuSearchBuilder::default()
                .iterations(1000)
                .tabu_list_size(30)
                .neighborhood_operator(Box::new(SwapNeighborhoodOperator::new(60)))
                .logger(Box::new(CSVLogger::new(instance, None)))
                .build()?,
        ),
    ])
